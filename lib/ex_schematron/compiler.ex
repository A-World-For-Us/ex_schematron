defmodule ExSchematron.Compiler do
  @moduledoc """
  Compiles a parsed schematron into the source of an Elixir validation module.

  The generated module transcribes the schematron; all XPath semantics live in
  ...
  @doc "Compiles a schematron file into formatted Elixir source for `module_name`."
  @spec compile_file!(Path.t(), module()) :: binary()
  def compile_file!(sch_path, module_name) do
    schema = Sch.parse_file!(sch_path)
    compile!(schema, module_name, generated_from: Path.basename(sch_path))
  end

  @spec compile!(Sch.Schema.t(), module(), keyword()) :: binary()
  def compile!(%Sch.Schema{} = schema, module_name, opts \\ []) do
    if schema.phases != [] do
    ...
    module_ast =
      quote do
        defmodule unquote(module_name) do
          ...
          # A schematron may declare helper functions that none of this file's rules
          # call; referencing them here keeps the unused-function check quiet.
          @doc false
    ...
    header = """
    # Generated by ex_schematron from #{opts[:generated_from] || "a schematron file"}. Do not edit.
    # Regenerate with: mix schematron.gen
    """

    source = header <> Macro.to_string(module_ast) <> "\n"

    source
    |> drop_unused_functions_alias()
    |> Code.format_string!(line_length: 135)
    |> IO.iodata_to_binary()
  end

  # A schematron whose expressions never hit the function library would leave
  # the F alias unused and make the generated module warn at compile time.
  defp drop_unused_functions_alias(source) do
    if String.match?(source, ~r/\bF\.\w/) do
      source
    else
      String.replace(source, ~r/ *alias ExSchematron\.Runtime\.Functions, as: F\n/, "")
    end
  end

  defp moduledoc(opts) do
    "Schematron validation checks generated from #{opts[:generated_from] || "a schematron file"}."
  end
